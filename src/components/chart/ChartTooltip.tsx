...
const classes = {
    root: "pointer-events-none absolute top-0 z-1 min-w-[var(--base-size-112)] -translate-x-1/2 rounded-[var(--border-radius-medium)] border-solid border-[length:var(--border-width-thin)] border-[color:var(--border-color-default)] bg-[var(--overlay-background-color)] p-[var(--base-size-8)] [box-shadow:var(--shadow-floating-small)]",
    label: "mb-[var(--base-size-4)] [color:var(--foreground-color-default)] [font-size:var(--text-body-size-small)] [font-weight:var(--base-text-weight-semibold)] [line-height:var(--text-body-line-height-small)]",
    ...
    name: "grow [color:var(--foreground-color-muted)]",
    // A column of numbers is the one place the digits are held to the same width, so that they
    // line up down the readout
    value: "[color:var(--foreground-color-default)] [font-variant-numeric:tabular-nums] [font-weight:var(--base-text-weight-semibold)]",
};
