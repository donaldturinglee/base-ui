...
// columns the whole way down. `contents` is the fallback for a browser without subgrid,
// which lays the cells out on the grid but drops the rows themselves
export const SUBGRID_CLASSES = [
    "contents",
    "supports-[grid-template-columns:subgrid]:grid",
    "supports-[grid-template-columns:subgrid]:grid-cols-subgrid",
    "supports-[grid-template-columns:subgrid]:[grid-column:-1/1]",
].join(" ");
