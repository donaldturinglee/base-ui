/**
 * The tree the installed hotkeys are held in. A sequence is stored a key to a level, so that what
 * has been typed so far is a position in the tree rather than a search through everything
 * installed: `g c` and `g i` share the `g` above them, and reaching a leaf is what fires.
 */
